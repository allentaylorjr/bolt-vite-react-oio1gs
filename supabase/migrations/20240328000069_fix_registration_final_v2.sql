...
-- Update any existing NULL values in a single transaction
BEGIN;
  UPDATE public.churches SET primary_color = '#2563eb' WHERE primary_color IS NULL;
  UPDATE public.churches SET button_text_color = '#FFFFFF' WHERE button_text_color IS NULL;
  UPDATE public.churches SET accent_color = '#FFFFFF' WHERE accent_color IS NULL;
  UPDATE public.churches SET font_family = 'Inter' WHERE font_family IS NULL;
COMMIT;
