...
BEGIN
  ...
  -- Generate valid slug
  church_slug := COALESCE(NEW.raw_user_meta_data->>'slug', lower(regexp_replace(church_name, '[^a-zA-Z0-9]+', '-', 'g')));
  church_slug := trim(both '-' from church_slug);
  
  IF church_slug = '' OR church_slug IS NULL THEN
    church_slug := 'my-church';
  END IF;
